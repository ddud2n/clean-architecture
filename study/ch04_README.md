# 4. 유스케이스 구현하기
* 현재 아키텍처에서 어플리케이션, 웹, 영속성 계층이 아주 느슨하게 결합
* 육각형 아키텍처는 도메인 엔티티를 만들고 엔티티를 중심으로 유스케이스 구현
...
public class Account { // Account 엔티티 = 실제 계좌의 현재 스냅샷을 제공
...
- 입력을 받는다.
    - `01. 입력을 받는다` 에서는 `입력 유효성 검증` 을 하지 않는다. 
    - 유스케이스 코드에서는 입력 유효성을 검증하지 않고, 도메인 로직에만 신경써야 함
